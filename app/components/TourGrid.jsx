...
export default function TourGrid({ tours, showCategoryFilter = true }) {
  ...
  const filtered = useMemo(() => {
    ...
    return [...list].sort((a, b) => {
      if (sort === "price-desc") return Number(b.priceLowest) - Number(a.priceLowest);
      if (sort === "az") return a.title.localeCompare(b.title);
      return Number(a.priceLowest) - Number(b.priceLowest);
    });
      ...
      {visible.length === 0 ? (
        <div className="rounded-2xl border border-dashed border-ink/15 py-20 text-center">
          ...
          <p className="mt-2 text-sm text-ink/55">
            Try a broader search — or just ask us, we build custom days all the time.
          </p>
          <button
            type="button"
            onClick={() => {
              ...
              <span key={n} className="flex items-center gap-1.5">
                ...
                <button
                  ...
                  className={`h-10 min-w-[2.5rem] rounded-full px-3 text-sm font-semibold transition ${
                    n === current
                      ? "bg-palm-600 text-white shadow-glow"
