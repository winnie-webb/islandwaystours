    ...
    <>
      ...
      <div className="hidden bg-ink text-white/70 lg:block">
        <div className="shell flex h-9 items-center justify-between text-xs">
          <p className="tracking-wide">
            Private tours, airport transfers &amp; cruise pickups — island-wide,
            seven days a week
          </p>
      ...
      <header
        className={`sticky top-0 z-50 transition-all duration-300 ${
          overHero
            ? "bg-transparent"
            : "border-b border-ink/[0.07] bg-white/85 shadow-[0_1px_24px_-12px_rgba(10,23,18,.25)] backdrop-blur-xl"
        }`}
      >
        <div className="shell flex h-[4.5rem] items-center gap-4 lg:h-20 lg:gap-8">
          ...
          <nav className="ml-2 hidden items-center gap-1 lg:flex">
            ...
            >
              ...
              {toursOpen && (
                <div className="absolute left-0 top-full w-[32rem] pt-3">
                    ...
                    <div className="grid grid-cols-2 gap-1">
                      {CATEGORIES.map((c) => (
                        <Link
                          ...
                          className="rounded-xl px-3 py-2.5 text-sm text-ink/75 transition hover:bg-palm-50 hover:text-palm-700"
                        >
                          {c.title}
                        </Link>
                      ))}
        ...
        <div className="fixed inset-0 z-[60] lg:hidden">
          ...
          <div className="absolute inset-y-0 right-0 flex w-full max-w-sm animate-fade-in flex-col bg-white shadow-lift">
            ...
            <div className="flex-1 overflow-y-auto px-5 py-6">
              ...
              <div className="mt-3 space-y-0.5">
                {CATEGORIES.map((c) => (
                  <Link
                    key={c.type}
                    href={`/category/${c.type}`}
                    className="block rounded-xl px-3 py-2.5 text-[0.95rem] text-ink/80 transition hover:bg-palm-50 hover:text-palm-700"
                  >
                    {c.title}
                  </Link>
                ))}
              ...
              <div className="mt-3 space-y-0.5">
                {[{ href: "/tours", label: "All Tours" }, ...COMPANY_LINKS].map(
                  (l) => (
                    <Link
                      key={l.href}
                      href={l.href}
              ...
              <div className="mt-8 flex gap-2.5">
                {[
                  ...
                  <a
                    ...
                    rel="noreferrer"
                    aria-label={label}
                    className="grid h-10 w-10 place-items-center rounded-full bg-ink/5 text-ink/70 transition hover:bg-palm-600 hover:text-white"
                  >
